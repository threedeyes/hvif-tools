...
bool
SVGParser::ParseFile(const std::string& svgFile, Icon& icon)
{
	...
	bool result = _ProcessImage(image, icon);
	nsvgDelete(image);
	return result;
}

bool
SVGParser::ParseString(const std::string& svgString, Icon& icon)
{
	std::vector<char> buffer;
	buffer.reserve(svgString.size() + 1);
	for (size_t i = 0; i < svgString.size(); ++i)
		buffer.push_back(svgString[i]);
	buffer.push_back('\0');
...
SVGParser::ParseBuffer(const char* svgData, size_t dataSize, Icon& icon)
{
	...
	return ParseString(svgString, icon);
}

bool
SVGParser::_ProcessImage(NSVGimage* image, Icon& icon)
{
	ConvertState state;
	state.icon = &icon;
...
void
SVGParser::_ProcessShape(NSVGshape* shape, ConvertState& state)
{
...
int
SVGParser::_ProcessPath(NSVGpath* path, ConvertState& state)
{
	if (path->npts < 2)
		return -1;

	Path iomPath;
	iomPath.closed = path->closed;
	...
	for (int i = 0; i < path->npts - 1; i += 3) {
		...
		if (i == 0) {
			ControlPoint cp;
			cp.x = p0_x * state.scale + state.tx;
			cp.y = p0_y * state.scale + state.ty;
			cp.x_in = cp.x;
			cp.y_in = cp.y;
			cp.x_out = c1_x * state.scale + state.tx;
			cp.y_out = c1_y * state.scale + state.ty;
			cp.connected = false;
			iomPath.points.push_back(cp);
		} else {
			iomPath.points[iomPath.points.size() - 1].x_out = c1_x * state.scale + state.tx;
			iomPath.points[iomPath.points.size() - 1].y_out = c1_y * state.scale + state.ty;
		}

		ControlPoint cp;
		cp.x = p1_x * state.scale + state.tx;
		cp.y = p1_y * state.scale + state.ty;
		cp.x_in = c2_x * state.scale + state.tx;
		cp.y_in = c2_y * state.scale + state.ty;
		cp.x_out = cp.x;
		cp.y_out = cp.y;
		cp.connected = false;
		iomPath.points.push_back(cp);
	...
	state.icon->paths.push_back(iomPath);
	return pathIndex;
}

int
SVGParser::_AddStyle(const NSVGpaint& paint, float opacity, ConvertState& state)
{
	Style style;
	
	if (paint.type == NSVG_PAINT_COLOR) {
		style.isGradient = false;
		style.color = _NSVGColorToIOM(paint.color, opacity);
		...
		if (paint.type == NSVG_PAINT_LINEAR_GRADIENT)
			...
			style.gradient.type = GRADIENT_CIRCULAR;

		style.gradient.interpolation = INTERPOLATION_LINEAR;
		style.gradient.inheritTransformation = true;
		
		for (int i = 0; i < grad->nstops; ++i) {
			ColorStop stop;
			stop.color = _NSVGColorToIOM(grad->stops[i].color, opacity);
		...
		style.isGradient = false;
		style.color = _NSVGColorToIOM(0x000000FF, opacity);
	}
	
	for (size_t i = 0; i < state.icon->styles.size(); ++i) {
		if (state.icon->styles[i] == style)
...
uint32_t
SVGParser::_NSVGColorToIOM(unsigned int color, float opacity)
{

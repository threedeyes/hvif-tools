...
std::string base64_encode(const unsigned char* bytes_to_encode, unsigned int in_len) {
    std::string ret;
    int i = 0; int j = 0; unsigned char char_array_3[3]; unsigned char char_array_4[4];
    while (in_len--) {
...
int main(int argc, char** argv) {
    ...
    svr.Post("/api/vectorize", [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string hvifJson = "";
        std::vector<uint8_t> svgBuffer(svgData.begin(), svgData.end());
        std::vector<uint8_t> hvifBuffer;
        haiku::ConvertOptions iconOpts;

        bool hvifOk = haiku::IconConverter::ConvertBuffer(

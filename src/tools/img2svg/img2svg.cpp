...
LoadBitmapData(const std::string& filename)
{
	...
	if (!roster) {
		std::cerr << "Error: Could not get translator roster" << std::endl;
		return BitmapData();
	}

	BBitmapStream stream;
	status_t result = roster->Translate(&file, NULL, NULL, &stream, B_TRANSLATOR_BITMAP);
	if (result != B_OK) {
		std::cerr << "Error: Could not translate image: " << filename << std::endl;
		return BitmapData();
	}
	...
	if (!bitmap || !bitmap->IsValid()) {
		std::cerr << "Error: Invalid bitmap from: " << filename << std::endl;
		delete bitmap;
		return BitmapData();
	}
	...
	BitmapData bitmapData(width, height, data);
	delete bitmap;
...
{
#ifdef __HAIKU__
	BApplication app("application/x-vnd.hvif-img2svg");
#endif

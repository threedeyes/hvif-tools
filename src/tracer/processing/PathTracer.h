...
public:
    ...
    std::vector<std::vector<std::vector<std::vector<double> > > >
			    BatchTracePaths(const std::vector<std::vector<std::vector<std::vector<double> > > >& internodePaths,
					float lineThreshold, float quadraticThreshold, SharedEdgeRegistry* registry = NULL);

private:
    std::vector<std::vector<double> >
			    _FitSequence(const std::vector<std::vector<double> >& path,
					float lineThreshold, float quadraticThreshold,
					int sequenceStart, int sequenceEnd, int depth, SharedEdgeRegistry* registry = NULL);
};
